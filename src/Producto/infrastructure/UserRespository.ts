...
export class productoRepositori implements ProductoRepository {
  async putUser(
    ...
    return await new Promise((resolve, reject) => {
      const sql = `UPDATE dbrest.users SET Nombre='${Nombre}', Telefono='${Telefono}' WHERE Correo='${Correo}'`;
  ...
  async deleteById(Correo: string): Promise<Producto | null> {
    ...
    return await new Promise((resolve, reject) => {
      mysql.connection.query(
        `DELETE FROM dbrest.users WHERE Correo = '${Correo}'`,
  ...
  async getById(Correo: string): Promise<Producto | null> {
    ...
    return await new Promise((resolve, reject) => {
      mysql.connection.query(
        `SELECT * FROM dbrest.users WHERE Correo = '${Correo}';`,
        (error: QueryError, rows: Producto) => {
          if (error) {
            reject(error);
          } else {
