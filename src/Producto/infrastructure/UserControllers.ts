...
export class ProductosController {
    ...
    create = async (req: Request, res: Response) => {
        try {
            ...
            if (!onlyLetters) {
                return res.status(400).json({ error: "Nombre solo debe llevar Letras" });
            }
            const onlyNumbers = /^[0-9]+$/.test(Telefono);
            if (!onlyNumbers) {
                return res.status(400).json({ error: "Telefono solo debe tener números" });
            }
            const onlycorreo =  /^\w+([.-_+]?\w+)*@\w+([.-]?\w+)*(\.\w{2,10})+$/.test(Correo);
            if (!onlycorreo) {
                return res.status(400).json({ error: "Correo solo debe tener estos simbolos ^\w+([.-_+]?\w+)*@\w+([.-]?\w+)*(\.\w{2,10})+$" });
            }
